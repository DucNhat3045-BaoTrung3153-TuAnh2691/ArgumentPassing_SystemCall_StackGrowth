...
start_process (void *file_name_)
{
 ...
 /* Parameter passing */
//khai báo con trỏ "esp" được đặt giá trị là trỏ stack của hàm hiện tại
	char* esp = (char*)if_.esp;
//khai báo 1 mảng "argv" và 1 số nguyên "argc" để theo dõi tham số được truyền cho hàm
	int argv[128];
	int argc = 0, tokenlen = 0;
//sd hàm strtok_r để tách chuỗi và trích xuất các tham số được truyền cho hàm
	for (; token != NULL; token = strtok_r(NULL, " ", &save_ptr)) {
   	 	tokenlen = strlen(token) + 1;
    		esp -= tokenlen;  //tạo chỗ trống trên stack
    		strlcpy(esp, token, tokenlen + 1); //sao chép token vào stack
    		argv[argc++] = (int)esp;//lưu địa chỉ của mỗi token trong mảng argv và tăng biến argc
	}	
//làm tròn con trỏ xuống bội số của 4 
	while ((int)esp % 4 != 0) {
    	esp--;
	}
//đẩy giá trị của mảng argv, biến argc và 1 giá trị null lên stack bằng cách cập nhật con trỏ esp
	int* tmp = (int*)esp;
	tmp--;
	*tmp = 0;
	tmp--;
...
//cập nhật con trỏ stack cho hàm hiện tại bằng giá trị của 1 con trỏ esp
	*tmp = (int)(tmp + 1);
	tmp--;
	*tmp = argc;
	tmp--;
